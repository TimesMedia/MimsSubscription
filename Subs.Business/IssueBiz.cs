...
namespace Subs.Business
{
    public static class IssueBiz
    {
        ...
        public static bool UnitsLeft(int pSubscriptionId, int pIssueId)
        {
            try
            {

                if (ProductDataStatic.GetUnitsLeft(pSubscriptionId, pIssueId) > 0)
                {
                    return true;
                }
                else 
        ...
        public static bool ForceDelivery(int pSubscriptionId, int pIssueId)
        {
            Subs.Data.SubscriptionDoc3.SubscriptionIssueDataTable lSubscriptionIssue = new SubscriptionDoc3.SubscriptionIssueDataTable();
            Subs.Data.SubscriptionDoc3TableAdapters.SubscriptionIssueTableAdapter lAdapter = new Data.SubscriptionDoc3TableAdapters.SubscriptionIssueTableAdapter();
            lAdapter.AttachConnection();
            ...
            try
            {
                ...
                if (lSubscriptionIssue.Count <= 1)
                {
                    ExceptionData.WriteException(1,"Issue " + pIssueId + " could not be found", "IssueBiz static","ForceDelivery","");
                    return false;
                }

                if (lSubscriptionIssue[0].UnitsLeft == 0)
                {
                    ExceptionData.WriteException(1, "Issue " + pIssueId + " not deliverable", "IssueBiz static", "ForceDelivery", "");
                    return false;
                }

                lSubscriptionIssue[0].ForceDelivery = true;
                lAdapter.Update(lSubscriptionIssue);
                return true;
            ...
            catch (Exception ex)
            {
                ...
                {
                    ExceptionLevel++;
                    ExceptionData.WriteException(1, ExceptionLevel.ToString() + " " + CurrentException.Message, "static IssueBiz", "ForceDelivery", "");
                    CurrentException = CurrentException.InnerException;
                } while (CurrentException != null);
