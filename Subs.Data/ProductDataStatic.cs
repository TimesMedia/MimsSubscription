...
namespace Subs.Data
{
    public static class ProductDataStatic
    {
        ...
        public static int GetUnitsLeft(int pSubscriptionId, int pIssueId)
        {
            try
            {
                gConnection.Open();
                ...
                Command.CommandText = "select UnitsLeft from SubscriptionIssue where SubscriptionId = " + pSubscriptionId.ToString()
                    + " and IssueId = " + pIssueId.ToString();
                int? Result = (int?)Command.ExecuteScalar();
                if (Result != null && Result.HasValue)
                {
                    return (int)Result;
            ...
            catch (Exception ex)
            {
                ...
                {
                    ExceptionLevel++;
                    ExceptionData.WriteException(1, ExceptionLevel.ToString() + " " + CurrentException.Message, "static ProductData", "GetUnitsLeft", "");
                    CurrentException = CurrentException.InnerException;
                } while (CurrentException != null);
