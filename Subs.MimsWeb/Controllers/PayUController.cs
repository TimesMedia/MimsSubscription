...
namespace Subs.MimsWeb.Controllers
{
    public class PayUController : Controller
    {
        ...
        private Subs.Data.PaymentDocTableAdapters.PayUResultTableAdapter gPayUResultAdapter = new Data.PaymentDocTableAdapters.PayUResultTableAdapter();
        ...
        public ActionResult IndexCommon(int pCustomerId, string pReference, Decimal pAmount)
        {
            ...
            try
            {
                ...
                soapContent += "<merchantReference>" + pReference + "</merchantReference>";
                //soapContent += "<merchantReference>" + lCustomerData.CustomerId.ToString() + "</merchantReference>";
                ...
                // Basket
                soapContent += "<Basket>";
                soapContent += "<amountInCents>" + Decimal.ToInt32(pAmount * 100) + "</amountInCents>";
                soapContent += "<currencyCode>ZAR</currencyCode>";
                soapContent += "<description>Order No: " + pReference + "</description>";
                soapContent += "</Basket>";
                soapContent += "</ns1:setTransaction>";
            ...
            catch (Exception ex)
            {
                ...
                {
                    ExceptionLevel++;
                    ExceptionData.WriteException(1, ExceptionLevel.ToString() + " " + CurrentException.Message, this.ToString(), "IndexCommon", "");
        ...
        public ActionResult Index()
        {
            ...
            (int lInvoiceId, decimal lAmount) = CustomerData3.GetOutstandingOnLastInvoice((int)lLoginRequest.CustomerId);

            if (lAmount < 0.05M)
            {
                ViewBag.Message = "There is nothing outstanding on your last invoice.";
                return View();
            }

           return IndexCommon((int)lLoginRequest.CustomerId, "INV" + lInvoiceId.ToString(), lAmount);
        }

        [HttpGet]
        public ActionResult IndexOwnAmount()
        {
            LoginRequest lLoginRequest = SessionHelper.GetLoginRequest(Session);
            int lCustomerId = (int)lLoginRequest.CustomerId;
            CustomerData3 lCustomerData = new CustomerData3(lCustomerId);
            PayUAmount lPaymentAmount = new PayUAmount() {Amount = lCustomerData.Due };
            return View(lPaymentAmount);
        }


        [HttpPost]
        public ActionResult IndexOwnAmount(decimal Amount)
        {
            LoginRequest lLoginRequest = SessionHelper.GetLoginRequest(Session);
            int lCustomerId = (int)lLoginRequest.CustomerId;
            return IndexCommon(lCustomerId, lCustomerId.ToString(), Amount);
        }
