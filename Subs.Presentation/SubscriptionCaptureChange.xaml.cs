...
namespace Subs.Presentation
{
    public partial class SubscriptionCaptureChange : Window
    {
        ...
        {
            IssuePicker lIssuePicker;
            try
            {
                
                if (gBasketItem.Subscription.ProductId == 0)
                {
                    MessageBox.Show("I cannot work with out a product.");
                    return;
                }


                lIssuePicker = new Subs.Presentation.IssuePicker(gBasketItem.Subscription.ProductId);
        ...
        private void buttonLastIssue_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                gBasketItem.Subscription.ProposedLastIssue = lResult.LastIssue;
                ...
                IssuePicker lIssuePicker = new Subs.Presentation.IssuePicker(gBasketItem.Subscription.ProductId);

                // Prime with the proposed Lastissue if this is a renewal
                if (gBasketItem.Subscription.ProposedLastIssue != 0)
                {
                    lIssuePicker.IssueSelect(gBasketItem.Subscription.ProposedLastIssue);
                }
                ...
                {
                    MessageBox.Show("You have not selected a last issue. Try again.");
                    return;
                };
        ...
        private void buttonReturn_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (gBasketItem.Subscription.ProposedLastIssue == 0)
                {
                    MessageBox.Show("You did not specify a last issue.");
                    return;
                }
