...
namespace Subs.Presentation
{
    ...
    public partial class SubscriptionPicker2 : Window
    {
        ...
        {
            Subs.Presentation.IssuePicker frmIssuePicker = new Subs.Presentation.IssuePicker(ProductId);
            frmIssuePicker.IssueSelect(gSubscriptionData.LastIssue);
        ...
        private void CancelSubscriptions_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    SubscriptionData3 lSubscription = new SubscriptionData3(lItem);
        ...
        private void BulkReturn_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ElicitString lElicitString = new ElicitString("Why do you want to return these subscriptions?");
                lElicitString.ShowDialog();
                if (lElicitString.Answer == "")
                {
                    return;
                ...
                ElicitRichText lElicit = new ElicitRichText("Please provide me with a list of tuples of subscriptionid - issueId - units. ");
                lElicit.ShowDialog();
                List<TupleOfIntegers> lListOfTupleOfIntegers = lElicit.ListOfTupleOfIntegers;
                lElicit.Close();
                MainWindow.Refresh();
                int lCounter = 0;
                Cursor = Cursors.Wait;

                foreach (TupleOfIntegers lItem in lListOfTupleOfIntegers)
                {
                    ...
                    if (!LedgerData.ReturnCheck(lItem.Integer1, lItem.Integer2))
                    {
...

                    {
                        ...
                        if ((lResult = SubscriptionBiz.ReverseDelivery(lSubscriptionData, lItem.Integer2, lItem.Integer3, lReason)) != "OK")
                        {
                    ...
                    decimal Money = lItem.Integer3 * lSubscriptionData.UnitPrice;
                    LedgerData.Return(lSubscriptionData.SubscriptionId, Money, lItem.Integer2, lItem.Integer3, "Delivery returned", lReason);

                    ProductDataStatic.PostReturn(lItem.Integer2, lItem.Integer3);
    
                    lCounter++;
                }
     
                    MessageBox.Show(lCounter.ToString() + " issues returned.");
            }
            catch (Exception ex)
            {
                //Display all the exceptions
                ...
                {
                    ExceptionLevel++;
                    ExceptionData.WriteException(1, ExceptionLevel.ToString() + " " + CurrentException.Message, this.ToString(), "BulkReturnClick_Click",
                                                 "SubscriptionId = " + lCurrentSubscriptionId.ToString());
                    CurrentException = CurrentException.InnerException;
                } while (CurrentException != null);
        ...
        private void CallPaymentAllocation()
        {
            int lCurrentSubscriptionId;
            ISubscriptionPicker lSubscriptionGrid = (ISubscriptionPicker)SubscriptionGrid.Content;
            if ((lCurrentSubscriptionId = lSubscriptionGrid.GetCurrentSubscriptionId()) == 0)
        ...
        private void NonDeliveryProcessing(NonDeliveryOptions pOption)
        {
            ...
            try
            {
                ...
                if (gSubscriptionData.Status == SubStatus.Cancelled)
                {
                    MessageBox.Show("You cannot return on a cancelled subscription. Try to refund the customer on his payment?");
                    return;
                }
