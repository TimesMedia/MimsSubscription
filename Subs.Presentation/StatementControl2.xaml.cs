    ...
    public partial class StatementControl2 : UserControl
    {
        ...
        private string Load(int pCustomerId, int pStatementId)
        {
            ...
            try
            {
                ...
                IEnumerable<Subs.Data.InvoicesAndPayments> lInvoice;  // Cater for the rest
                lInvoice = lPaymentAndInvoice.Where(p => !(p.OperationId == (int)Operation.Pay || p.OperationId == (int)Operation.Refund || p.OperationId == (int)Operation.ReversePayment))
