...
namespace Subs.Presentation
{
    ...
    public partial class ProFormaControl : UserControl
    {
        ...
        public ProFormaControl()
        {
            InitializeComponent();
        }

        public string LoadAndRenderInvoice(int pPayerId, int pProFormaId)
        {
            string lStage = "Start";

            List<int> lSubscriptionIds = ProFormaData.GetSubscriptionIds(pProFormaId);

            try
            {
                gCustomerData = new CustomerData3(pPayerId);
                ...
                InvoiceNumber.Content = "PRO" + pProFormaId;
                ...
                InvoiceDate.Content = DateTime.Now.ToString("dd MMM yyyy");
                ...
                foreach (int lSubscriptionId in lSubscriptionIds)
                {
                    lStage = "Items - CurrentSubscriptionId = "+ lSubscriptionId.ToString();

                    SubscriptionData3 lSubscription = new SubscriptionData3(lSubscriptionId);
                ...
                string lFileName = "PRO"
                + pProFormaId.ToString()
                + ".pdf";
            ...
            catch (Exception ex)
            {
                ...
                {
                    ExceptionLevel++;
                    ExceptionData.WriteException(1, ExceptionLevel.ToString() + " " + CurrentException.Message, this.ToString(), "LoadAndRenderInvoice", lStage);
